...
if __name__ == '__main__':
    ...
    # saveAllTIds()
    id_dict_epoch=getAllTIdsFromEpoch("log/log-tweetIds-in-db-2017-and-epoch.txt")

    filestoeval = []
    for root, dir, files in os.walk("TRECdataset/submissions/scenarioA"):
        for file in files:
            filestoeval.append(file)
    for eachfile in filestoeval:
         id_str_filtered = ""
         with open("TRECdataset/submissions/scenarioA/"+eachfile, "r") as f:
            lines = f.readlines()
            for line in lines:
                id = line.strip().split()[1]
                if id in id_dict_epoch:
                    id_str_filtered += line
         ft.write_to_file("TRECdataset/submissions/scenarioAF/" + eachfile + "-U", id_str_filtered)
         print("Update file to: ", "TRECdataset/submissions" + eachfile + "-U")